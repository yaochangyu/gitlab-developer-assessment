...
**輸出檔案：**
- `./output/users/{username}/analysis-result.md` - 個別開發者分析報告
- `./output/users/all-user-analysis-result.md` - 所有開發者彙總報告 🆕

**彙總報告特色：**
- ✅ Markdown 表格格式，按總分降序排列
- ✅ 包含所有維度評分（7 個維度）
- ✅ 等級分佈統計（高級/中級/初級工程師比例）
- ✅ 分數統計（平均分、最高分、最低分）
- ✅ 各維度平均分
- ✅ 方便複製到 Excel 進行進一步分析
...
**彙總報告範例：**
```markdown
| 使用者名稱 | 總分 | 等級 | 程式碼貢獻量 | Commit 品質 | 技術廣度 | ... |
|-----------|------|------|-------------|------------|---------|-----|
| alice | 8.52 | 高級工程師 | 9.00 | 8.50 | 10.00 | ... |
| bob | 6.85 | 中級工程師 | 7.00 | 6.80 | 8.00 | ... |
```
...
- ✨ 支援批次分析多位開發者
- ✨ **自動產生彙總報告** - `all-user-analysis-result.md` 🆕
  - Markdown 表格格式，按總分降序排列
  - 等級分佈統計（高級/中級/初級工程師比例）
  - 分數統計（平均分、最高分、最低分）
  - 各維度平均分
