...
#### Step 3: 掃描目錄確認 CSV 檔案

**⚠️ 不要依賴 index.md，直接掃描目錄！**
...
# 或使用更友善的格式
find ./output/users/<username>/ -name "*.csv" -type f
```
...
for user in "${USERS[@]}"; do
  if [ ! -d "./output/users/$user" ] || [ -z "$(ls -A ./output/users/$user/*.csv 2>/dev/null)" ]; then
    echo "❌ $user: 資料不存在或不完整"
    MISSING_USERS+=("$user")
  else
    echo "✅ $user: 資料已存在，跳過收集"
  fi
done
...
```
   ...
   > 💡 **最佳實踐**：
   > - 先掃描 `.\scripts\output\users\{username}\` 目錄確認是否已有資料
   > - 若目錄不存在或 CSV 檔案不完整，才執行 `gl-cli.py user-details`
   > - 避免重複收集造成資源浪費
...
5. **讀取並解析輸出檔案**
   ...
   - ⚠️ **不要依賴 `{username}-index.md`**，直接掃描目錄確認實際 CSV 檔案

### 第 2.5 步：掃描 CSV 檔案（必要步驟）

**⚠️ 重要：在進行資料分析前，必須先掃描目錄確認實際存在的 CSV 檔案！**

**工作流程**：
1. 檢查使用者目錄是否存在：`.\scripts\output\users\{username}\`
2. 若不存在 → 返回「第 2 步」執行 `gl-cli.py user-details` 收集資料
3. 若存在 → 掃描目錄下所有 CSV 檔案

```bash
# 1. 檢查使用者目錄是否存在
USER_DIR=".\scripts\output\users\<username>"

if [ -d "$USER_DIR" ]; then
  echo "✅ 使用者目錄存在，繼續分析流程"
else
  echo "❌ 使用者目錄不存在：$USER_DIR"
...
# 或使用更詳細的方式
find "$USER_DIR" -name "*.csv" -type f -exec ls -lh {} \;
```

**⚠️ 為什麼不使用 index.md？**

索引檔案（`{username}-index.md`）在每次執行 `gl-cli.py user-details` 時都會被重新生成，只記錄**當次匯出的檔案**。如果分多次執行收集不同資料，索引檔案會不完整。

**範例問題情境**：
```bash
# 第一次執行：收集 commits, code_changes 等資料
python3 gl-cli.py user-details --username G2023018
# 產生：commits.csv, code_changes.csv, merge_requests.csv, ...
# index.md 包含：全部 7 個檔案

# 第二次執行：單獨收集 user_profile（例如更新個人資訊）
python3 gl-cli.py user-details --username G2023018
# 產生：user_profile.csv, permissions.csv
# index.md 被覆蓋：只剩 2 個檔案 ❌

# 結果：目錄中有 9 個 CSV 檔案，但 index.md 只列出 2 個
```

**正確做法**：直接掃描目錄
```bash
# 範例：掃描開發者 G2023018 的所有 CSV 檔案
ls -lah .\scripts\output\users\G2023018\*.csv

# 輸出範例：
# -rwxrwxrwx 1 yao yao  20M Jan 21 15:10 code_changes.csv
# -rwxrwxrwx 1 yao yao 186K Jan 21 15:10 code_reviews.csv
# -rwxrwxrwx 1 yao yao 144K Jan 21 15:10 commits.csv
# -rwxrwxrwx 1 yao yao  63K Jan 21 15:10 merge_requests.csv
# -rwxrwxrwx 1 yao yao  206 Jan 21 18:18 permissions.csv
# -rwxrwxrwx 1 yao yao  552 Jan 21 15:10 statistics.csv
# -rwxrwxrwx 1 yao yao 693K Jan 21 15:10 user_events.csv
# -rwxrwxrwx 1 yao yao  630 Jan 21 18:18 user_profile.csv
```

**後續分析步驟**：
根據目錄掃描結果，依序讀取需要的 CSV 檔案進行深入分析：
